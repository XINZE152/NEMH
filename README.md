...
**总览**：两个 Vite + React 前端 + 一个 Express API，统一经 **`/api`** 对接；开发用 dev server 反向代理，生产用网关 / Nginx 转发。

```
┌──────────────────┐         ┌──────────────────┐
│      Client      │         │      Admin       │
│   Vite + React   │         │ Vite + React + UI│
└────────┬─────────┘         └────────┬─────────┘
         │         /api               │
         └──────────────┬──────────────┘
                        ▼
  ...
  开发环境 (Node)                    生产环境
  /api → Dev Server 反向代理         /api → 网关 / Nginx 转发 → Node
```

| 组件 | 说明 |
|------|------|
| **Client** | Vite + React，浏览器请求以 `/api` 为前缀访问后端 |
| **Admin** | Vite + React + UI 库，同上 |
| **Server** | Express + 数据库，为两端提供统一 API |

- **开发**：各前端 dev server 将 **`/api` 反向代理**到本地 Node 后端。
- **生产**：由**网关或 Nginx** 将 **`/api`** 转发到 Node 进程（与开发期路径约定一致）。
